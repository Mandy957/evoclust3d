...
class ScopeAlgorithm:                    
    ...
    def __init__(self, alignmentSet):
        ...
        self.BranchName = re.compile("<Branch_name>(.+?)</Branch_name>").search(self.alignmentSet).group(1)
    ...
    def getAllMutationXMLKeysAccordingToAccession(self,AccKey):
        #print self.mutationsXML_D[self.mutationsXMLkey_L[0]]
        #print self.getAccessionPosition_L(self.mutationsXML_D[self.mutationsXMLkey_L[0]])
        
        Ret = []
        for mutationsXMLKey in self.mutationsXMLkey_L:
            L = self.getAccessionPosition_L(self.mutationsXML_D[mutationsXMLKey])
            if L != None:
                for i in L:
                    if i[0] == AccKey:
                        if mutationsXMLKey in set(Ret):
                            pass
                        else:
                            Ret.append(mutationsXMLKey)
